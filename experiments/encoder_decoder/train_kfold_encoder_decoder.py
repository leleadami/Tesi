"""
K-Fold Cross Validation - LSTM Encoder-Decoder

Usage:
    python train_kfold_encoder_decoder.py --building_id 1 --target cooling_demand --k_folds 5 --num_epochs 50
    python train_kfold_encoder_decoder.py --hidden_size 256 --building_id 2
    python train_kfold_encoder_decoder.py --target solar_generation
"""
...
def load_config() -> dict:
    """Load encoder-decoder model configuration"""
    config_path = Path(__file__).parent.parent.parent / 'configs' / 'encoder_decoder.yaml'
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='K-Fold Cross Validation - LSTM Encoder-Decoder',
    ...
    print("=" * 80)
    print("TIME SERIES CROSS VALIDATION - LSTM ENCODER-DECODER")
    print("=" * 80)
    ...
    fig, axes = plot_kfold_scatter(
        ...
        title=f"LSTM Encoder-Decoder - {args.target.replace('_', ' ').title()} - {n_folds}-Fold Sliding Window",
    ...
    fig, ax = plot_scatter_predictions(
        ...
        title=f"LSTM Encoder-Decoder - {args.target.replace('_', ' ').title()}\n{n_folds}-Fold Sliding Window CV",
    ...
    if kfold_config['results']['save_individual_folds']:
        for i, (y_true, y_pred) in enumerate(zip(y_true_folds, y_pred_folds), 1):
            fig, ax = plot_scatter_predictions(
                y_true, y_pred,
                title=f"LSTM Encoder-Decoder - Fold {i}",
