"""
K-Fold Sliding Window CV - Transformer

Usage:
    python train_kfold_transformer.py --building_id 1 --target cooling_demand --k_folds 5 --num_epochs 50
    python train_kfold_transformer.py --d_model 256 --building_id 2
    python train_kfold_transformer.py --target solar_generation
"""
...
def load_config() -> dict:
    ...
    config_path = Path(__file__).parent.parent.parent / 'configs' / 'transformer.yaml'
...
def main():
    parser = argparse.ArgumentParser(
        description='K-Fold Sliding Window CV - Transformer',
    ...
    parser.add_argument('--d_model', type=int, default=None,
                       help='Model dimension for Transformer (overrides config, default: 128)')
    ...
    print("=" * 80)
    print("TIME SERIES CROSS VALIDATION - TRANSFORMER")
    print("=" * 80)
    ...
    fig, axes = plot_kfold_scatter(
        ...
        title=f"Transformer - {args.target.replace('_', ' ').title()} - {n_folds}-Fold Sliding Window",
    ...
    fig, ax = plot_scatter_predictions(
        ...
        title=f"Transformer - {args.target.replace('_', ' ').title()}\n{n_folds}-Fold Sliding Window CV",
