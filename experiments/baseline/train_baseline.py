...
Examples:
  ...
  # Train on Phase 1 with custom hidden size
  python train_baseline.py --hidden_size 128

  # Train on Phase 2 online (2208h) Building 1
  python train_baseline.py --phase phase_2_online --building_id 1

  # Train on Phase 2 online Building 2 with custom hyperparameters
  python train_baseline.py --phase phase_2_online --building_id 2 --hidden_size 128 --lr 0.0005

  # Full custom configuration
  python train_baseline.py --phase phase_2_online --building_id 3 --hidden_size 256 --batch_size 64 --lr 0.0008 --dropout 0.3
...
Checkpoints saved to: results/checkpoints/{phase}/
    ...
    # Create model
    model = create_lstm_model(input_size=input_size, config=config['model'])
    print(f"\n✓ Model created")
    ...
    trainer = Trainer(
        ...
        experiment_name='model'  # Just use 'model' since directory name already has full info
    )
    ...
    config_save = {
        ...
        'phase': phase,
        'building_id': building_id,
        'timestamp': experiment_dir.name.split('_')[-2] + '_' + experiment_dir.name.split('_')[-1],  # Extract timestamp from name
        'model': {
            'type': 'LSTM_Baseline',
